...
**Критичные сочетания трёх и более параметров.** Покрытие всех пар не гарантирует
проверку взаимодействий трёх и более факторов. Ниже перечислены сочетания,
критичные для процессинга, и указано, какими строками наборов они покрыты:

| Сочетание                                                | Почему критично                                      | Покрывающая строка      |
| -------------------------------------------------------- | ---------------------------------------------------- | ----------------------- |
| ACTIVE + превышение дневного лимита + превышение баланса | Порядок проверок определяет код отказа (61, а не 51) | TC-PW-AUTH-09, -14, -19 |
| ACTIVE + истёкший срок + превышение баланса              | Срок проверяется раньше баланса → код 54             | TC-PW-AUTH-11, -17, -21 |
| INACTIVE + истёкший срок                                 | Статус первичен → CARD_INACTIVE, а не 54             | TC-PW-AUTH-03           |
| reserve + DELETED-карта                                  | Удалённая карта не участвует в транзакциях           | ограничение 6 модели CM |
| create + невалидный BIN + неверный формат expiryDate     | Две независимые ошибки валидации в одном запросе     | TC-PW-CM-04             |
